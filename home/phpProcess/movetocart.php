<?php
   ...
   if(isset($_SESSION["username"]))
   {
      ...
      $q1="SELECT * FROM wish WHERE CustomerID='$cust_id' AND ProductID='$pid'";
      $res1=mysqli_query($con,$q1) or die("q1 error");
      if($res1)
      {
         while($row1=mysqli_fetch_array($res1))
         {
            //qry to check whether product already in cart
            $qry="SELECT * FROM cart WHERE CustomerID='$cust_id' AND ProductID='$pid'";
            ...
            if($result)
            {
               ...
               {//product already exists in cart
                  ...
                  //query to update cart; since, user can change quantity and add to cart
                  $query="UPDATE cart SET ProductPrice=$row1[ProductPrice],Quantity=$row1[Quantity],
               ...
               {//product is not already in cart
                  $q2="INSERT INTO cart(CustomerID,ProductID,ProductPrice,Quantity,vegan,FinalPrice)
      ...
      //query to delete product from wishlist
      $q3="DELETE FROM wish WHERE CustomerID='$cust_id' AND ProductID='$pid'";
